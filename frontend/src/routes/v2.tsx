...
function AppV2() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const parsedOverrides = storedOverrides ? (JSON.parse(storedOverrides) as CourseOverrides) : {}
      const safeOverrides: CourseOverrides =
        parsedOverrides && typeof parsedOverrides === 'object' ? parsedOverrides : {}
  ...
  const handleSwapCourse = useCallback((courseKey: string, replacement: UHCourse | null) => {
        ...
        <div className="relative z-10 space-y-10 rounded-[32px] border border-slate-800/60 bg-slate-900/70 p-10 shadow-2xl backdrop-blur">
          ...
          <section className="space-y-5 rounded-3xl border border-slate-800/70 bg-slate-950/60 p-6 shadow-inner">
            <header className="space-y-1">
              ...
              <h2 className="text-xl font-semibold text-slate-50">Search university pathways</h2>
              <p className="text-sm text-slate-400">
                Start from an official UH pathway and customize it, or skip this step for a
                fully generated plan.
              </p>
            </header>
            ...
            <form onSubmit={handlePathwaySearch} className="flex flex-col gap-3 md:flex-row">
            ...
            {selectedPathway ? (
              <div className="flex flex-col gap-2 rounded-2xl border border-sky-500/30 bg-sky-500/10 p-4 text-slate-100 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-300">Selected</p>
              ...
              <p className="text-sm text-slate-400">
                No pathway selected. We&apos;ll build a plan from scratch unless you pick one.
              </p>
            )}
            ...
            {pathwayQuery ? (
              <div className="space-y-3">
                {isSearchingPathways ? (
                  ...
                  <p className="rounded-2xl border border-slate-800/70 bg-slate-950/50 px-4 py-3 text-sm text-slate-300">
                    No pathways found for “{pathwayQuery}”. Try another keyword or switch search
                    modes.
                  </p>
                )}
              </div>
              ...
              <p className="text-sm text-slate-500">
                Enter a keyword to browse matching pathways. Text search is instant; comprehensive
                search digs deeper but may take longer.
              </p>
            )}
          </section>
          ...
          <section className="space-y-3 rounded-3xl border border-slate-800/70 bg-slate-950/60 p-6 shadow-inner">
            <header className="space-y-1">
              ...
              <h2 className="text-xl font-semibold text-slate-50">Launch a personalized plan</h2>
              <p className="text-sm text-slate-400">
                Share your interests and goals. We&apos;ll generate or refine the
                pathway using this context.
              </p>
            </header>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  >
                    Attach supporting files <span className="font-normal text-slate-500">(optional)</span>
                  </label>
                  <p className="text-xs text-slate-500">
                    PDF, DOCX, or TXT transcripts, syllabi, or transfer evaluations help tailor your plan.
                  </p>
                </div>
                <input
                  id="supporting-files"
                ...
                <p className="text-xs text-slate-500">
                  Files are only processed when launching a new plan (not when refining a saved pathway).
                </p>
              </div>

              <div className="flex flex-col items-center justify-between gap-4 text-sm text-slate-500 md:flex-row">
                <span>
                  Tip: mention preferred subjects, transfer credits, or time constraints for a sharper plan.
                </span>
                <button
                  type="submit"
          ...
          {planToShow ? (
            <section className="space-y-6 rounded-3xl border border-slate-800 bg-slate-950/40 p-8">
              <div className="space-y-2">
                ...
                <h2 className="text-3xl font-semibold text-slate-50">{planToShow.program_name}</h2>
                <p className="text-sm uppercase tracking-[0.3em] text-slate-500">
                  {planToShow.institution} &middot; {planToShow.total_credits} credits
                </p>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {planToShow.candidates.map((candidate) => (
                      <button
                        ...
                        type="button"
                        onClick={() => handleCandidatePathwayClick(candidate.pathway_id)}
  ...
  return (
    <article
      className={`rounded-2xl border p-4 shadow-inner ${
        isSelected ? 'border-sky-400 bg-sky-500/5' : 'border-slate-800/70 bg-slate-950/40'
      }`}
    >
          ...
          <div className="space-y-6">
            ...
            <div className="max-h-[50vh] space-y-4 overflow-y-auto pr-2">
              {data.years.map((year) => (
                ...
                >
                  <p className="text-sm font-semibold text-slate-200">Year {year.year_number}</p>
        ...
        {data && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-slate-300">
              Generating from this pathway will refresh the current plan using your existing
              prompt.
            </p>
        ...
        <div className="sticky top-0 z-10 border-b border-slate-800 bg-slate-950/95">
          <div className="flex items-center justify-between px-6 py-4">
            <div>
              ...
              <h2 className="text-lg font-semibold text-slate-100">{plan.program_name}</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
              ...
              {pages.map((page) => {
                if (page.type === 'overview') {
                  return (
                    ...
                    >
                      <div className="space-y-3">
                        <h3 className="text-3xl font-semibold text-slate-50">{plan.program_name}</h3>
                        <p className="text-sm uppercase tracking-[0.3em] text-slate-500">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {plan.candidates.map((candidate) => (
                              <button
                                key={candidate.pathway_id}
                                type="button"
                                onClick={() => onGeneratePathway(candidate.pathway_id)}
                                className="rounded-full border border-slate-700 bg-slate-900/60 px-4 py-1 text-xs font-semibold uppercase tracking-wide text-slate-200 transition hover:border-sky-500 hover:text-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-400/60 focus:ring-offset-2 focus:ring-offset-slate-950"
                              >
                                {candidate.name}
                              </button>
                    ...
                    {hasCourses ? (
                      <div className="grid gap-4 md:grid-cols-2">
                        {semester.courses.map((course, index) => {
                          ...
                          const activeCourse = courseOverrides[courseKey] ?? course
...
function CourseCard({
  ...
  const candidateAlternatives =
    sourceCourse.candidates?.filter((candidate) => !areCoursesEqual(candidate, course)) ?? []
    ...
    <div className="space-y-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-5 shadow-inner">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <h4 className="text-lg font-medium text-slate-100">{course.course_title}</h4>
        </div>
...
const buildCourseKey = (
  ...
  const identifier = course.course_id ?? `${course.course_prefix}-${course.course_number}-${suffix}`
    ...
    years: plan.years.map((year) => ({
      ...
      semesters: year.semesters.map((semester) => ({
        ...
        courses: semester.courses.map((course, index) => {
          const courseKey = buildCourseKey(year.year_number, semester.semester_name, index, course)
      ...
      `${year.year_number}:${year.semesters
        .map((semester) =>
          `${semester.semester_name}:${semester.courses
            .map((course, index) =>
              buildCourseKey(year.year_number, semester.semester_name, index, course),
            )
            .join(',')}`,
        )
        .join('|')}`,
