...
import type { CourseOverrides, CourseSelection, Theme } from "@/utils/pathway";
...
function AppV2() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => setIsHeroVisible(true));
        ...
        <div className="relative w-full max-w-5xl">
          <nav className="mb-6 flex items-center justify-between rounded-2xl border border-slate-800/60 bg-slate-950/60 px-6 py-4 shadow-lg backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-base text-lg font-semibold text-slate-100 text-sky-300">
                  AlaAI
                </p>
              </div>
            </div>
          ...
          <div className="relative z-10 space-y-10 rounded-[32px] bg-slate-900/70 p-10 shadow-2xl backdrop-blur">
            <h1
              className={`text-center text-[100px] font-semibold tracking-[0.1em] leading-none bg-clip-text text-transparent transition-all duration-700 ease-out ${
                isHeroVisible
              ...
              style={{
                backgroundImage:
                  "linear-gradient(120deg, #c1e9ffff, #3499cfff , #0369a1",
                backgroundSize: "200% 200%",
                animation: "alaaiGradientShift 5s ease infinite",
              }}
            >
              AlaAI
            </h1>
            <header className="space-y-3 text-center">
              <div className="flex items-center justify-center gap-2 text-sky-400">
                <Sparkles
                  className={`h-5 w-5 transition-all duration-700 ease-out ${
                    isHeroVisible
                      ? "translate-y-0 opacity-100"
                      : "translate-y-3 opacity-0"
                  }`}
                ...
                />
                <span
                  className={`text-sm font-semibold uppercase tracking-[0.3em] transition-all duration-700 ease-out ${
                    isHeroVisible
                      ? "translate-y-0 opacity-100"
                      : "translate-y-3 opacity-0"
                  }`}
              ...
              <h1
                className={`text-4xl font-semibold tracking-tight transition-all duration-700 ease-out ${
                  isHeroVisible
                    ? "translate-y-0 opacity-100"
                    : "translate-y-3 opacity-0"
                }`}
              ...
              <p
                className={`mx-auto max-w-2xl text-base text-slate-400 transition-all duration-700 delay-150 ease-out ${
                  isHeroVisible
                    ? "translate-y-0 opacity-100"
                    : "translate-y-3 opacity-0"
                }`}
            ...
            <section className="space-y-5 rounded-3xl border border-slate-800/70 bg-slate-950/60 p-6 shadow-inner">
              <header className="space-y-1">
                ...
                <p className="text-sm text-slate-400">
                  Start from an official UH pathway and customize it, or skip
                  this step for a fully generated plan.
                </p>
              </header>
              ...
              {selectedPathway ? (
                ...
                <p className="text-sm text-slate-400">
                  No pathway selected. We&apos;ll build a plan from scratch
                  unless you pick one.
                </p>
              )}
                  ...
                  {isSearchingPathways ? (
                    ...
                    pathwayResults.map((pathway) => (
                      <PathwayResultCard
                        ...
                        onPreview={(pathwayId) =>
                          setPreviewPathwayId(pathwayId)
                        }
                      />
                    ))
                  ) : (
                    <p className="rounded-2xl border border-slate-800/70 bg-slate-950/50 px-4 py-3 text-sm text-slate-300">
                      No pathways found for “{pathwayQuery}”. Try another
                      keyword or switch search modes.
                    </p>
                  )}
                </div>
              ) : null}
